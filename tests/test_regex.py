...
result = [
    ("/path/to/{this}.{set}.py", '/path/to/(?P<this>[A-Za-z0-9]+).(?P<set>[A-Za-z0-9]+).py'),
    ("/path/{another}o/{this}.{set}.py", '/path/(?P<another>[A-Za-z0-9]+)o/(?P<this>[A-Za-z0-9]+).(?P<set>[A-Za-z0-9]+).py'),
    ("/path/to/{this}.{set:[A-Z_]+}.py", '/path/to/(?P<this>[A-Za-z0-9]+).(?P<set>[A-Z_]+).py')
]
...
def test_compile_regex_should_handle_mixed_default_and_overrides(string_template, expected_regex):
