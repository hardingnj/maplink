...
def compile_regex_from_source(
    source_string_: str,
    match_underscore: bool = False,
    match_period: bool = False,
    match_hyphen: bool = False,
) -> re.Pattern:
...
def determine_and_validate_targets(
    filepaths: Iterable[str], target_string: str, regex: re.Pattern
) -> dict[str, str]:
    targets = dict()
    ...
    for filepath in filepaths:
        ...
        if target_ in targets:
            raise RuntimeError(
                f"Duplicate target ({target_}) found. From {targets[target_]} and {filepath}. "
...
def create_symlink(
    source_path: Path, target_path: Path, clobber=False, absolute_path=True
):
...
def cli(
    ...
    target_dict = determine_and_validate_targets(
        glob.glob(glob_pattern), target_template, compiled_pattern
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create symlinks based on a match string and output format."
    )
    parser.add_argument(
        "source_template",
        help='The pattern to match file paths, e.g., "/path/{group1:[A-Za-z0-9-]}_{group2}_{read}_L11.gz".',
    )
    parser.add_argument(
        "target_template",
        help='The format for the output symlink paths. e.g. "/path/{group1}_{group2}_{read}_L11.gz".',
    )
    parser.add_argument(
        "--create",
        action="store_true",
        help="Create directories and symlinks instead of just printing the message.",
    )
    parser.add_argument(
        "--clobber",
        action="store_true",
        help="Clobber existing symlinks.",
        default=False,
    )
    parser.add_argument(
        "--match-underscore",
        action="store_true",
        help="Match underscores (_) by default (in addition to alphanumerics).",
        default=False,
    )
    parser.add_argument(
        "--match-period",
        action="store_true",
        help="Match periods (.) by default (in addition to alphanumerics).",
        default=False,
    )
    parser.add_argument(
        "--match-hyphen",
        action="store_true",
        help="Match hyphens (-) by default (in addition to alphanumerics).",
        default=False,
    )
